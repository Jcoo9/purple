...
(function(){
  ...
  var SKEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNud3Z4ZGJnYnZsb3VieXBobXR3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQ3NTM1NTMsImV4cCI6MjA2MDMyOTU1M30.TV_-5S3PW_ECTFL7qCMcOL7NBSJTQNuMy6RhM0FLBB8';
  ...
  style.textContent=[
    ...
    '#'+UID+' .pp-sides{display:grid;grid-template-columns:1fr 1fr;background:'+(DARK?'#252118':'#141210')+';cursor:pointer}',
    '#'+UID+' .pp-side{padding:8px 10px}',
    ...
    '#'+UID+' .pp-btns{display:grid;grid-template-columns:1fr 1px 1fr;border-bottom:1px solid '+(DARK?'#3a3530':'#d4cdc2')+'}',
    ...
    '#'+UID+' .pp-nuance{display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid '+(DARK?'#3a3530':'#d4cdc2')+'}',
    '#'+UID+' .pp-nuance-btn{padding:8px;cursor:pointer;background:transparent;border:none;font-family:monospace;font-size:7px;letter-spacing:.06em;text-transform:uppercase;color:'+(DARK?'#8a837a':'#7a736a')+';width:100%;transition:background .15s}',
  ...
  fetch(query,{headers:{'apikey':SKEY,'Authorization':'Bearer '+SKEY}})
    ...
    .then(function(topics){
      ...
      var scored=topics.filter(function(t){return tv(t)>=2&&!voted[t.id];}).map(function(t){
        var tot=tv(t),yesPct=Math.round(((t.votes_yes||0)/tot)*100),gap=Math.abs(yesPct-50);
