...
(function(){
  ...
  function render(){
    ...
    wrap.innerHTML=
      ...
      '<div class="pp-q">'+
        ...
        esc(t.question)+
      '</div>'+
      '<div class="pp-sides" id="'+UID+'-sides-'+t.id+'" onclick="window[\'ppExpand_'+UID+'\']()">'+
        '<div class="pp-side">'+
          '<div class="pp-side-lbl" style="color:'+yesClr+'">← Left argues</div>'+
          '<div class="pp-side-txt" id="'+UID+'-l-'+t.id+'" style="-webkit-line-clamp:2">Tap to expand both arguments →</div>'+
        '</div>'+
        '<div class="pp-side-div"></div>'+
        '<div class="pp-side">'+
          '<div class="pp-side-lbl" style="color:'+noClr+'">Right argues →</div>'+
          '<div class="pp-side-txt" id="'+UID+'-r-'+t.id+'" style="-webkit-line-clamp:2">Tap to expand →</div>'+
        '</div>'+
      '</div>'+
      (!hasVoted?
        '<div class="pp-btns">'+
  ...
  window['ppExpand_'+UID]=function(){
  var t=questions[curQ];
  if(!t)return;
  var el=document.getElementById(UID+'-sides-'+t.id);
  if(!el)return;
  var expanded=el.dataset.expanded==='1';
  if(expanded){
    el.dataset.expanded='0';
    document.getElementById(UID+'-l-'+t.id).style.webkitLineClamp='2';
    document.getElementById(UID+'-r-'+t.id).style.webkitLineClamp='2';
  } else {
    el.dataset.expanded='1';
    document.getElementById(UID+'-l-'+t.id).style.webkitLineClamp='unset';
    document.getElementById(UID+'-r-'+t.id).style.webkitLineClamp='unset';
    // Load full party positions if not loaded yet
    loadSides(t);
  }
};
